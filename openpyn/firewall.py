...
def apply_fw_rules(interfaces_details, vpn_server_ip):
    ...
    subprocess.call("sudo iptables -A INPUT -i lo -j ACCEPT", shell=True)
    subprocess.call("sudo iptables -A OUTPUT -o lo -j ACCEPT", shell=True)

    # best practice, stops spoofing
    subprocess.call("sudo iptables -A INPUT -s 127.0.0.0/8 -j DROP", shell=True)

    # Default action if no other rules match
    subprocess.call("sudo iptables -P OUTPUT DROP", shell=True)
    subprocess.call("sudo iptables -P INPUT DROP", shell=True)
    return
